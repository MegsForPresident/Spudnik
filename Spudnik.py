...
@client.command()
async def alert(ctx,member :discord.Member, *,reason=None):
    await member.send(f'{ctx.author.mention} wants to alert you about \n{reason}')
...
async def mute(ctx, member : discord.Member, hours=0, minutes=10, seconds=0, *,reason=None):
    ...
    guild = ctx.guild
    print("mute")
    data['Member'][str(member.id)]['Mutes'] += 1
    save(data) 
...
async def on_member_join(member:discord.Member):
    ...
    data['Member'][str(member.id)] = memberData
...
async def on_member_remove(member:discord.Member):
    ...
    def determine():
        if data['Member'][str(member.id)]['Bans']:
            return 'Dead'
        return 'Left'
    to = determine()
    data[to][str(member.id)] = data['Member'][str(member.id)]
    del data['Member'][str(member.id)]
...
async def ban(ctx, member :discord.Member, *,reason=None):
    print(reason)
    data['Member'][str(member.id)]['Bans'] = True
...
async def on_message(message):
    ...
    if(message.author == client.user):
        return
    elif not len(curses) == 0 and not "spud." in message.content and (curse in message for curse in curses):
...
client.run('ODcwNjY5Mjc2ODcxMjYyMjE4.YQQH8w.rpuXinE3q2yfxoEUKtW5LjLVKlY')